<?php if($session['tipo'] == 0){?>
	<div class="fab" data-target="#newcajaModal" data-toggle="modal"> + </div>
<?php } ?>
<div class="row">
	<h2 class="text-center" style="margin:20px 0 1px 0;font-weight:300">LISTA DEL CAJAS DE LA CIUDAD DE POTOSÍ</h2>
</div>
<div class="row" style="margin:10px"> <!-- SECTION TABLE USERS -->
	<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
		<div class="col-md-12">
	          <ul class="nav nav-tabs nav-justified" id="myTabs">
	               <li role="presentation" estado="caja" class="active"><a href="#todos" aria-controls="todos" role="tab" data-toggle="tab">ACTIVOS<span class="badge" style="background:red;margin-left:10px;color:#fff"><?php echo count($resultado["cajas"]);?></span></a></li>
	               <li role="presentation" estado="cajaretirado"><a href="#baja" aria-controls="baja" role="tab" data-toggle="tab">RETIRADOS<span class="badge" style="background:red;margin-left:10px"><?php echo count($resultado["cajasretirados"]);?></span></a></li>
	          </ul>
	     </div>
	          ...
	          <div id="todos" role="tabpanel" class="tab-pane active">
				<?php for ($i=0; $i <count($resultado["cajas"]); $i++) {?>
					<?php $i%2==0 ? $color="#0d8990": $color="#484848";?>
						<div class="col-md-4 col-sm-6 col-xs-12 col-lg-3" type="button"  data-target="#updatecajaModal" data-toggle="modal" onclick="updateAjax(<?php echo $resultado["cajas"][$i]['id'];?>,'caja')" style="border:2px solid #fff;cursor:pointer;background:<?php echo $color;?>">
	          ...
	          <div id="baja" role="tabpanel" class="tab-pane">
				<?php for ($i=0; $i <count($resultado["cajasretirados"]); $i++) {?>
					<?php $i%2==0 ? $color="#bd673e": $color="#484848";?>
						<div class="col-md-4 col-sm-6 col-xs-12 col-lg-3" type="button"  data-target="#updatecajaModal" data-toggle="modal" onclick="updateAjax(<?php echo $resultado["cajasretirados"][$i]['id'];?>,'cajaretirado')" style="border:2px solid #fff;cursor:pointer;background:<?php echo $color;?>">
							<div class="col-md-4 col-sm-4 col-xs-4" style="padding:0;margin:0">
								<img style="margin:20px 0 15px 0;padding:0" src="<?php echo URL;?>public/images/icons/64/box1.png">
							</div>
							<div class="col-md-8 col-sm-8 col-xs-8" style="padding:0;margin:0">
...
<?php 	include 'modalnewcaja.php';
		include 'modalupdatecaja.php';?>
<script>
	var id_caja_p,table_p;
    $(document).ready(function(){
		$('#btnregistrar').click(function(){$.ajax({url: '/Caja/crear',type: 'post',data:{tabla:$('#selectcaja option:selected').val()=="Cajon" ? ("caja"):("cajaretirado"),nombre:$('#selectcaja option:selected').val()},success:function(obj){if (obj=="false") {$('#error_registro').show();}else{swal("Mensaje de Alerta!", obj , "success");setInterval(function(){ window.location.href = "/Caja"; }, 1500);}}});});
	});
	function updateAjax(val,table){
		table_p=table;$("#btneliminar_u").hide();
		...
		swal({
			...
			$.ajax({
				...
				success:function(obj){
					...
					setInterval(function(){ window.location.href = "/Caja"; }, 1000);
				}
			});
		});
	}
